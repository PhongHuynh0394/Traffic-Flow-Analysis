...
CLICKHOUSE_CONN_ID = "conn_clickhouse"
CLICKHOUSE_TABLE = "dim_cam_info"
CLICKHOUSE_DATABASE = "traffic_flow"
...
def trans__raw__dim_cam_info():
    from utils.clickhouse.filetoclickhouse import File2Clickhouse
    ...
    with SparkIO(conf=spark_conf, gcs=True) as spark:
        ...
        # Save cleaned data to Clickhouse
        dim_cam_pd = dim_cam_info_cleaned.toPandas()
        ch_ingester = File2Clickhouse(
            conn_id=CLICKHOUSE_CONN_ID
        )

        try:
            ch_ingester.ingest_to_clickhouse(
                data=dim_cam_pd,
            ...
            logging.info(f"Ingested {len(dim_cam_pd)} rows to Clickhouse")
        except Exception as e:
            logging.error(f"Failed to write data to Clickhouse, error: {e}")
            raise
